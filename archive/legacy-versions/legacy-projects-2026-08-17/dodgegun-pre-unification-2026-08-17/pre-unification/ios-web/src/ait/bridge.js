...
export function loadSettings() {
  ...
  return { sound: true, haptic: true, nickname: 'PLAYER', ws: '' };
}
...
export async function lockLandscape() {
  native({ type: 'awake', enabled: true });
  try {
    await screen.orientation?.lock?.('landscape');
  } catch { /* iOS native lock */ }
}

export async function restorePortrait() {
  native({ type: 'awake', enabled: false });
  try {
...
export async function notifyMatchEnded() {
  native({ type: 'match_end' });
}

export async function haptic(kind = 'light') {
  native({ type: 'haptic', kind: kind === 'heavy' ? 'heavy' : 'light' });
  try {
...
export async function getUserKey() {
  ...
  if (!key) {
    key = `ios-${Math.random().toString(36).slice(2, 10)}`;
...
export async function getGameProfile() {
  ...
  const nickname = String(settings.nickname || '').trim() || 'PLAYER';
...
export async function submitUserWinScore() {
  return { ok: true };
}
