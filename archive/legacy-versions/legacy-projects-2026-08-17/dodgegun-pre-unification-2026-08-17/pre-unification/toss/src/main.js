...
function setScreen(name) {
  ...
  if (name === 'home' || name === 'settings') stopBgm();
  if (name !== lastScreen) {
    lastScreen = name;
    trackScreen(name);
  }
...
async function startPvp() {
  ...
  if (!ok) {
    $('result-tag').textContent = '빠른 대전';
    $('result-title').textContent = '연결 실패';
    $('result-score').textContent = '';
    $('result-reason').textContent = net.state.error || '서버에 연결할 수 없어요';
    $('btn-rematch').textContent = '다시 찾기';
    stopBgm();
    setScreen('result');
  }
}
...
$('btn-leaderboard').addEventListener('click', async () => {
  ...
  if (!opened) {
    const stats = loadStats(userKey);
    banner(`로컬 기록 · 유저전 ${stats.userWins}승`);
    setTimeout(() => { if (screen === 'home') banner(''); }, 1600);
  }
});
...
$('btn-settings-back').addEventListener('click', () => setScreen('home'));
