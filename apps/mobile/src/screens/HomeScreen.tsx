...
const tabs: Array<{ key: HomeTab; label: string; blurb: string; cta: string }> = [
  { key: 'quick', label: '빠른 대전', blurb: '접속 중인 상대와 자동 매칭', cta: '대국 시작' },
  { key: 'friend', label: '친구', blurb: '입장코드로 친구와 대국합니다', cta: '코드 만들기' },
  { key: 'ai', label: '컴퓨터', blurb: '난이도와 진영을 골라 연습합니다', cta: '대국 준비' },
  { key: 'local', label: '같이 두기', blurb: '한 기기에서 흑·백을 번갈아 둡니다', cta: '대국 시작' },
];
    ...
    <View style={styles.screen}>
      <Screen>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: spacing.sm, paddingTop: 4 }}>
          <Text style={[typography.display, { color: colors.ink }]}>몽진</Text>
          <Pressable onPress={openProfile} style={{ marginLeft: 'auto', alignItems: 'flex-end', backgroundColor: colors.surface, borderRadius: 14, borderWidth: 1, borderColor: colors.line, paddingHorizontal: 12, paddingVertical: 6 }}>
            ...
            <Text style={{ color: colors.inkSoft, fontSize: 11, marginTop: 2 }}>Elo {visibleProfile.rating} · {visibleProfile.wins}승</Text>
          </Pressable>
        </View>
