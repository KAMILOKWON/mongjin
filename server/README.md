...
`IDENTITY.profile`, `PROFILE.profile`, 대국 결과의 `profile`, 토스 로그인 응답의 `profile`에
`hasPlayedMove: boolean`을 포함한다. 서버가 받은 합법적인 사람의 `MOVE`가 처음 처리되면
프로필에 `true`를 영구 저장하고 연결된 세션에 `PROFILE`을 보낸다. 승패·Elo·매칭에는 영향을 주지 않는다.
첫 수 전에 항복해 생긴 승패만으로는 `true`가 되지 않는다.

클라이언트의 로컬 비랭크 연습에서는 실제 합법 수를 적용한 후, 인증된 WebSocket 연결로
`{ "type": "FIRST_MOVE_PLAYED" }`를 한 번 보낼 수 있다. 서버는 메시지 본문을 사용하지 않고
첫 착수 여부만 저장해 `PROFILE`로 응답한다. 로컬 착수 자체는 서버가 검증할 수 없으므로 이 표시는
보안·부정행위 방지 증거가 아니다. 이전 클라이언트의 `HELLO`와 `GET_PROFILE`은 그대로 동작한다.

시작할 때 저장된 기보와 결과·대국 이벤트를 `matchId`로 연결해 기존 계정의 표시를 복원한다.
봇 대국은 기록에 저장된 사람 진영을 확인한다. 사람 간 대국의 두 수 이상은 양쪽을 확인하며,
한 수만 남은 대국은 기록된 승자·진영을 연결할 수 있을 때만 흑을 확인한다.
